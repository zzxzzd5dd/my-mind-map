...
export default {
  ...
  computed: {
     ...
     * @Desc: 设置toolbar状态
     */
    // 不是本人创建返回true，系统创建返回true
    // 
    isNotCreator() {
      if(this.activeNodes[0].nodeData.data._creatorPoliceNumber === this.userInfo.account && !this.activeNodes[0].nodeData.data._isSystemDefault) {
        return false
      }
      else {
        return true
